...
function drawChart1() {
        ...
        data.setValue(0,2,means[0]);
        data.setValue(0,3,means[1]);
        data.setValue(29,2,means[0]);
        data.setValue(29,3,means[2]);
        var options = {
            ...
            interval: {
                'i0': { 'color':'red', 'style':'bars', 'barWidth':0,'lineWidth':20, 'pointSize':0, 'fillOpacity':1},
                'i1': { 'color':'black', 'style':'bars', 'barWidth':0,'lineWidth':0, 'pointSize':0, 'fillOpacity':1}
...
function makeData(mean_diff) {
    ...
    if (SSG < SST) {
        ...
        document.getElementById("ms_btwn2").innerHTML = SSG.toFixed(2);
...
function resetDiff() {
    document.getElementById("diff_btwn_means2").value = 4;
    drawChart2();
}
