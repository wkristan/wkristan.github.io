...
## Likelihood with a discrete probability distribution - the Poisson

Read through the Poisson distribution material and answer the following questions.

#### What makes the Poisson a "discrete" probability distribution?

> The outcomes are all counts, so they can only be non-negative integer numbers.

#### Since the Poisson is discrete, is it possible to calculate the probability of a single observation, like a count of white blood cells on a smear of 2? Or is the "probability" really a probability density?

> Yes, since each outcome is discrete we can calculate the probabilities. Continuous distributions have heights only, so we can only calculate probability density for continuous distributions, but for discrete distributions we can calculate probabilities.

#### The likelihood function and the probability distribution function are the same for the Poisson. How is it possible then for the likelihood function to be continuous, but for the probability distribution function to be discrete?

> The mean of a set of counts can be any decimal number, so the likelihood function for possible values of the mean is a continous function.
...
> It's 119.42, because 119 is very close to it.

**Question: for which of these data values is the mean estimate of 119.4 least likely?**

> 135.61, becuase 119 is far from that data value.
...
> Because it is far away from most of the data values.
...
> No, it's not at the peak for any single data value, but it's fairly high on every curve. Across the whole set of values it has the highest likelihood collectively.
...
> No, being in the middle of the data was enough for it to maximize the likelihood across the entire data set.
...
> No, likelihoods can't be interpreted in isolation like that. It is the highest likelihood for any estimate, and that's all that matters.
...
> You need to minimize the negative log-likelihood to maximize the likelihood.
...
> It is the same shape, and has the location of the best estimate in the same location, it is just flipped over.
...
read_excel("likelihood_data.xlsx", sheet = "EstimateMean") -> mean.data
...
> No, R has a much greater precision.
...
> The best will be close to an intercept of 5.31 and a slope of 0.267.  
...
> Yes, there is only one maximum, even though we are using a 3D surface.
...
> The estimates are correlated, they work together - the best slope depends on the intercept used, and vice versa. The combinations that work well are a narrow range of values, which make the surface narrow.
...
> It is higher, because we're getting closer to the data.
...
read_excel("likelihood_data.xlsx",sheet="Regression") -> regression
...
lm(y ~ x, data = regression) -> linear.lm
linear.lm
...
We can profile the likelihood function to obtain confidence intervals for a proportion. Head back to Canvas to the proportion estimator app.
